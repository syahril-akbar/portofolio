        ...
        <div class="flex flex-col md:flex-row items-center justify-between gap-12">
            <div class="md:w-3/5 space-y-6">
                ...
                <div class="flex flex-wrap gap-4 pt-2">
                    @php
                        $waPhone = preg_replace('/[^0-9]/', '', $profile->phone);
                        if (str_starts_with($waPhone, '0')) {
                            $waPhone = '62' . substr($waPhone, 1);
                        }
                    @endphp
                    <a href="https://wa.me/{{ $waPhone }}" target="_blank" class="px-7 py-2.5 bg-teal-600 text-white rounded-lg font-medium hover:bg-teal-700 transition-all shadow-md transform hover:-translate-y-0.5 flex items-center gap-2">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L0 24l6.335-1.662c1.72.937 3.659 1.432 5.623 1.432h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg>
                        Hubungi Saya
                    </a>
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div onclick="openProjectModal('{{ $project->slug }}')" role="button" tabindex="0" class="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 group flex flex-col cursor-pointer">
                        ...
                        @else
                            <div class="w-full h-full flex items-center justify-center text-gray-400">
                                <div class="text-center">
                                    ...
                                    <p class="text-gray-400 text-sm mb-2">Belum ada gambar</p>
                                    @if($project->demo_link || $project->github_link)
                                        <a href="{{ $project->demo_link ?? $project->github_link }}" target="_blank" class="text-teal-600 text-sm font-medium hover:text-teal-800 transition-colors">Lihat Preview →</a>
                                    @endif
                                </div>
                            </div>
                        @endif
...
<!-- Modal -->
<div id="projectModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="modalTitle" onclick="event.target==this&&closeProjectModal()">
    <div class="absolute inset-0 bg-black/60 backdrop-blur-sm" onclick="closeProjectModal()"></div>
    <div class="relative bg-white rounded-2xl max-w-6xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        <div class="sticky top-0 bg-white border-b border-gray-100 px-8 py-4 flex justify-between items-center">
            <h2 id="modalTitle" class="text-2xl font-bold text-gray-900 outfit-font">Project</h2>
            <button onclick="closeProjectModal()" class="text-gray-500 hover:text-gray-700 text-2xl leading-none px-2 transition-colors focus:outline-none focus:ring-2 focus:ring-teal-500 rounded" aria-label="Tutup modal">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
        </div>
        <div id="modalContent" class="px-8 py-6"></div>
    </div>
</div>

<script>
const projectsData = @json($projects->keyBy('slug'));
...
function openProjectModal(slug) {
    ...
    const techStack = Array.isArray(p.tech_stack) ? p.tech_stack : [];
    const techHtml = techStack.length ? `
        <div class="mb-6">
            <h3 class="text-sm font-bold text-gray-500 uppercase tracking-wider mb-3">Tech Stack</h3>
            <div class="flex flex-wrap gap-2">
                ${techStack.map(t => `<span class="px-3 py-1 bg-gray-100 text-gray-700 text-sm font-medium rounded-md">${esc(t)}</span>`).join('')}
            </div>
        </div>` : '';

    const dateRange = (p.start_date || p.end_date) ? `
        <div class="mb-6">
            <h3 class="text-sm font-bold text-gray-500 uppercase tracking-wider mb-2">Periode</h3>
            <p class="text-gray-700">${formatDate(p.start_date)} - ${p.end_date ? formatDate(p.end_date) : 'Sekarang'}</p>
        </div>` : '';

    const roleHtml = p.role ? `
        <div class="mb-6">
            <h3 class="text-sm font-bold text-gray-500 uppercase tracking-wider mb-2">Peran</h3>
            <p class="text-teal-600 font-medium">${esc(p.role)}</p>
        </div>` : '';

    const linksHtml = (p.demo_link || p.github_link) ? `
        <div class="flex flex-wrap gap-4 pt-4 border-t border-gray-100">
            ${p.demo_link ? `<a href="${esc(p.demo_link)}" target="_blank" class="inline-flex items-center gap-2 px-5 py-2.5 bg-teal-600 text-white rounded-lg font-medium hover:bg-teal-700 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                Live Demo
            </a>` : ''}
            ${p.github_link ? `<a href="${esc(p.github_link)}" target="_blank" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gray-800 text-white rounded-lg font-medium hover:bg-gray-900 transition-colors">
    ...
    const noImageHtml = p.demo_link || p.github_link ? `
        <div class="mb-6 rounded-xl bg-gradient-to-br from-gray-50 to-gray-100 border-2 border-dashed border-gray-200 flex flex-col items-center justify-center py-12 px-6 text-center">
            ...
            ${(p.demo_link || p.github_link) ? `<a href="${esc(p.demo_link || p.github_link)}" target="_blank" class="inline-flex items-center gap-2 px-5 py-2 bg-teal-600 text-white rounded-lg text-sm font-medium hover:bg-teal-700 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                Lihat Preview
            </a>` : ''}
        </div>` : '';
